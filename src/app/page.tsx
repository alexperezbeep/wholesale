  ...
  return (
    <main className="min-h-screen bg-white text-black dark:bg-zinc-900 dark:text-white font-sans" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <section className="w-full bg-[var(--card)] dark:bg-zinc-800 rounded-2xl shadow-sm mx-auto mt-12 mb-10 max-w-3xl p-10 text-center">
        <h1 className="text-4xl md:text-5xl font-bold mb-4 text-[var(--primary)]">Need to Sell Your Home Fast & Easy?</h1>
        <p className="text-lg font-medium text-[var(--text)] mb-6">Cash offer. No repairs. Move on your timeline.</p>
        <button className="bg-[var(--primary)] hover:bg-[var(--accent)] dark:bg-[var(--accent)] dark:hover:bg-[var(--primary)] text-white font-bold py-3 px-8 rounded-lg shadow text-lg transition">Get Offer</button>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-[var(--card)] dark:bg-zinc-800 rounded-xl p-6 text-center shadow hover:scale-105 transition">
      ...
      {/* PROPERTY PHOTOS */}
      <section className="max-w-7xl mx-auto px-4 py-12">
        <h2 className="text-3xl font-bold text-center mb-10">Recent Properties</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {['s1.jpg', 's2.jpg', 's3.jpg', '516-518.jpg', 'zz.jpg', '109.png'].map((img, i) => (
            <div key={i} className="aspect-[4/3] rounded-lg overflow-hidden shadow bg-[var(--card)]">
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-[var(--card)] dark:bg-zinc-800 p-6 rounded-lg shadow text-center">
              <img src={`/${t.img}`} alt={t.name} className="rounded-full w-20 h-20 object-cover mx-auto border-4 border-[var(--primary)] mb-3" />
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="space-y-4">
          {faqs.map(({ q, a }, i) => (
            <div key={i} className="bg-[var(--card)] dark:bg-zinc-800 border border-[var(--primary)] rounded-xl shadow-sm">
              <button
                id={`faq-btn-${i}`}
